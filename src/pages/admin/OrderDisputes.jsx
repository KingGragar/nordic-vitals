...
import { getAdminOrderDisputes, resolveAdminOrderDispute } from '../../api/mlmApi'
...
function ResolveModal({ dispute, onResolve, onClose }) {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={onClose}>
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 28, maxWidth: 540, width: '100%' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          <div style={{ fontWeight: 700, fontSize: 16 }}>Order Dispute — {dispute.orderId}</div>
        ...
        <div style={{ background: 'var(--bg)', borderRadius: 8, padding: 14, marginBottom: 16, fontSize: 13 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            <div><span style={{ color: 'var(--text2)' }}>Customer:</span> <strong>{dispute.memberName}</strong></div>
            <div><span style={{ color: 'var(--text2)' }}>Reason:</span> {REASON_LABELS[dispute.reason] || dispute.reason}</div>
            <div><span style={{ color: 'var(--text2)' }}>Claimed refund:</span> <strong style={{ color: '#86efac' }}>{dispute.claimedRefund}</strong></div>
            <div><span style={{ color: 'var(--text2)' }}>Filed:</span> {dispute.filedAt}</div>
          </div>
          <div style={{ color: 'var(--text2)', fontSize: 12 }}>Description:</div>
          <div style={{ marginTop: 4, lineHeight: 1.5 }}>{dispute.description}</div>
        </div>
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <label style={lbl}>Decision</label>
            <select value={decision} onChange={e => setDecision(e.target.value)} style={inp}>
              <option value="resolved">Resolve — approve refund / replacement</option>
              <option value="denied">Deny — order was correct</option>
              <option value="reviewing">Keep in Review</option>
            </select>
          </div>
          <div>
            <label style={lbl}>Internal Note</label>
            <textarea value={note} onChange={e => setNote(e.target.value)} rows={3} style={{ ...inp, resize: 'vertical' }} placeholder="Reason for this decision…" />
          </div>
...
export default function AdminOrderDisputes() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    getAdminOrderDisputes().then(setDisputes).finally(() => setLoading(false))
  }, [])
  useEffect(() => { load() }, [load])

  async function handleResolve(id, payload) {
    await resolveAdminOrderDispute(id, payload)
    setDisputes(d => d.map(x => x.id === id ? { ...x, status: payload.decision } : x))
  }
    ...
    <AdminLayout>
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px' }}>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontWeight: 800, fontSize: 22, marginBottom: 4 }}>📦 Order Disputes</div>
          <div style={{ color: 'var(--text2)', fontSize: 14 }}>Manage customer disputes about wrong items, damage, or delivery issues.</div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12, marginBottom: 22 }}>
          {[
            ...
            { label: 'In Review', value: stats.reviewing },
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ color: 'var(--text2)', borderBottom: '2px solid var(--border)' }}>
                  {['Order', 'Customer', 'Reason', 'Refund', 'Filed', 'Status', ''].map(h => (
                ...
                {filtered.map(d => {
                  ...
                  return (
                    <tr key={d.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      <td style={{ padding: '10px 10px', fontWeight: 600, color: 'var(--gold)' }}>{d.orderId}</td>
                      <td style={{ padding: '10px 10px' }}>{d.memberName}</td>
                      <td style={{ padding: '10px 10px', color: 'var(--text2)' }}>{REASON_LABELS[d.reason] || d.reason}</td>
                      <td style={{ padding: '10px 10px', color: '#86efac', fontWeight: 600 }}>{d.claimedRefund}</td>
      ...
      {modal && <ResolveModal dispute={modal} onResolve={handleResolve} onClose={() => setModal(null)} />}
    </AdminLayout>
  )
}
